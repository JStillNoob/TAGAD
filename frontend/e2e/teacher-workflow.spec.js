...
test('teacher uploads, refreshes an active session, simulates data, reports, and logs out', async ({ page }, testInfo) => {
  ...
  await expect(page.getByRole('heading', { name: 'Camera & Worker Health' })).toBeVisible()
  await expect(page.getByText('Official analytics', { exact: true })).toBeVisible()
  await expect(page.getByText('Waiting for the camera controller.')).toBeVisible()
